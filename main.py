...
def parse_arguments():
    ...
    parser.add_argument("--start_timesteps", default=10e3, type=int, help="Random exploration timesteps")
    parser.add_argument("--eval_freq", default=10e3, type=int, help="Evaluation frequency")
    ...
    parser.add_argument("--policy_noise", default=0.1, type=float, help="Target policy smoothing noise")
    parser.add_argument("--noise_clip", default=0.5, type=float, help="Noise clipping range")
    parser.add_argument("--policy_freq", default=2, type=int, help="Delayed policy update frequency")
    ...
    parser.add_argument("--distributional", default=True, type=lambda x: str(x).lower() in ['true', '1', 'yes'], 
                       help="Use distributional critic")
    parser.add_argument("--num_atoms", default=101, type=int, help="Number of atoms for distributional critic")
    parser.add_argument("--v_min", default=-250.0, type=float, help="Min value for distributional critic")
    parser.add_argument("--v_max", default=250.0, type=float, help="Max value for distributional critic")
    parser.add_argument("--normalize_obs", default=True, type=lambda x: str(x).lower() in ['true', '1', 'yes'], 
                       help="Normalize observations")
    parser.add_argument("--lr_schedule", default=None, choices=[None, 'linear', 'cosine'], 
...
def train_vectorized(args, env, eval_env, policy, replay_buffer, obs_normalizer):
    ...
    while total_timesteps < args.max_timesteps:
        ...
        if total_timesteps % 10000 == 0 and total_timesteps > 0:
            ...
            print(f"Training progress: {total_timesteps}/{args.max_timesteps} timesteps, SPS: {sps:.1f}")
        ...
        replay_buffer.add_batch(states, actions, next_states, rewards, terminations.astype(np.float32))
        ...
        if total_timesteps % args.eval_freq == 0:
            ...
            eval_reward = evaluate_policy(policy, eval_env_to_use, eval_episodes=10, 
...
def main():
    ...
    # Scale training updates with number of environments for consistent learning
    if args.num_updates == 1:  # Only auto-scale if using default
        # Balance between training efficiency and computational cost
        # With many envs, we don't need 1:1 ratio due to better data diversity
        args.num_updates = min(16, max(1, args.num_envs // 8))
    ...
    obs_normalizer = utils.EmpiricalNormalization(shape=(state_dim,)) if args.normalize_obs else None
    policy = create_policy(args, state_dim, action_dim, max_action)
    
    # Load pretrained model if specified
    if args.load_model:
        model_path = f"{args.policy}_{args.env}_{args.seed}" if args.load_model == "default" else args.load_model
