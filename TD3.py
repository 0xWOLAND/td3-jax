...
class FastTD3:
    ...
    def __init__(
        ...
        self._store_hyperparameters(max_action, discount, tau, policy_noise, 
                                   noise_clip, policy_freq, action_dim)
    ...
    def _create_lr_schedules(self, lr_schedule, actor_lr, critic_lr, total_timesteps):
        ...
        if lr_schedule == 'linear':
            ...
            actor_schedule = optax.cosine_decay_schedule(
                init_value=actor_lr,
                decay_steps=int(total_timesteps)
            )
    ...
    def _store_hyperparameters(self, max_action, discount, tau, policy_noise, 
                             noise_clip, policy_freq, action_dim):
    ...
    def _categorical_projection(self, next_probs, rewards, not_dones):
        ...
        target_probs = target_probs.at[batch_indices.flatten(), indices_l.flatten()].add(d_l.flatten())
        target_probs = target_probs.at[batch_indices.flatten(), indices_u.flatten()].add(d_u.flatten())
    ...
    def _critic_loss_distributional(self, critic_params, target_params, batch, rng):
        ...
        target_logits_1, target_logits_2 = self.critic.apply(target_params['critic'], next_state, next_action)
    ...
    def _critic_step_distributional(self, critic_params, critic_opt_state, target_params, batch, rng):
    ...
    def _actor_step_distributional(self, actor_params, actor_opt_state, critic_params, state):
        loss, grads = jax.value_and_grad(self._actor_loss_distributional)(actor_params, critic_params, state)
    ...
    def _train_step(self, batch):
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            self.actor_target_params = self.update_targets(self.actor_params, self.actor_target_params)
            self.critic_target_params = self.update_targets(self.critic_params, self.critic_target_params)
